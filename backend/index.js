...
app.post("/api/ai/chat", protect, async (req, res, next) => {
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(200).json({
        success: true,
        reply: "I am a simple fallback bot. Configure the OpenRouter API key in the backend for real conversational capabilities."
      });
    }
    ...
    const aiResponse = await axios.post(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert HR assistant. Answer questions concisely and professionally."
          },
          {
            role: "user",
            content: message
